...
export const updatePcNameFromDB= async ( pc_name, pcId) =>{
    const [rows] = await pool.execute(
        "UPDATE created_pcs SET pc_name= ? WHERE pc_id = ?", [pc_name ,pcId]
    )
    return rows.affectedRows > 0 //devuelve true si edita el pc y false si no pudo
}

export const updateComponentFromDB= async (componentId, componentName, type, price, image) => {
    const [rows] = await pool.execute(
...
export const updateComponentsInPc = async (cpu, gpu, ram, storage, motherboard, psu, pcCase, cooler, pcId) => {
    // los componentes obligatorios deben estar presentes
    if (!cpu || !psu || !ram || !storage || !motherboard) {
